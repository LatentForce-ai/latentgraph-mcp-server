/**
 * AGENTS.md generation for Latentgraph latent-code integration.
 *
 * latent-code loads AGENTS.md automatically as project-level instructions,
 * equivalent to CLAUDE.md in Claude Code. This file injects the mandatory
 * lgraph MCP usage rules into every latent-code session for the project.
 */
...
This project has five Latentgraph subagents you can invoke with \`@\`:

| Agent | Purpose |
|-------|---------|
| \`@lgraph-exploring\` | Architecture exploration, module ownership, code flow |
| \`@lgraph-editing\` | Bug fixes, features, refactors — with blast-radius awareness |
| \`@lgraph-impact\` | Change safety analysis and downstream impact assessment |
| \`@lgraph-debugging\` | Trace failures through the dependency graph |
| \`@lgraph-cli\` | Latentgraph CLI operations (init, status, rescan) |
