// Integration orchestrator — generates latent-code integration files:
...
async function askForConsent(projectRoot: string, consentOverride?: boolean): Promise<boolean> {
    ...
    console.log('\n  Latentgraph wants to set up latent-code integration in your project:\n');
...
export async function setupLatentCodeIntegration(
    projectRoot: string,
    ...
    if (options.hasUserConsent === true) {
        ...
        if (!accepted) {
            console.log(
                '\n  Skipped. Run "lgraph add latent-code" at any time to set up latent-code integration.\n',
            );
        ...
        return {
            ...
            skipReason:
                ...
                'Run "lgraph add latent-code" to set up interactively, ' +
                'or pass --yes to approve automatically in scripts.',
        };
    }

    console.log('  Setting up latent-code integration...\n');

    createAgentsMd(projectRoot);
    console.log('  Tip: commit AGENTS.md so your team shares the setup.');
    console.log('  Note: Hook functionality is not yet supported for latent-code.');
