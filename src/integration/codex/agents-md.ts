...
|-------------|-------------------|
| "Explain the project" | \`get_context(targets=['project'])\` → \`get_context(targets=['<module-or-file>'])\` → \`get_file\` |
| "What does this file do?" | \`get_file\` |
| "What module owns this?" | \`get_context(targets=['<file-path>'])\` |
| "What does this depend on?" | \`get_dependencies\` |
| "What uses this?" | \`get_change_impact\` |
| "Fix/refactor this file" | \`get_file\` → \`get_dependencies\` → \`get_change_impact\` |
| "Change this subsystem" | \`get_context(targets=['<file-path>'])\` → \`get_file\` → \`get_dependencies\` → \`get_change_impact\` |
...
- Do not pass \`project_id\` unless you explicitly need to override the project; the MCP server already injects it from environment.
