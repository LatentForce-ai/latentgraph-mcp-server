/**
 * AGENTS.md generation for Latentgraph Codex integration.
 *
 * Creates or updates AGENTS.md in the project root with mandatory
 * usage rules that guide Codex toward using lgraph MCP tools.
 */
...
|-------------|-------------------|
| "Explain the project" | \`get_project_overview()\` → \`get_module_info(module_path='<module>')\` → \`get_file(file_path='<file>')\` |
| "What does this file do?" | \`get_file\` |
| "What module owns this?" | \`get_file\` (returns \`module_name\`) → optional \`get_module_info\` |
| "What does this depend on?" | \`get_dependencies\` (read \`outgoing\`) |
| "What uses this?" | \`get_dependencies\` (read \`incoming\` — that's the blast radius) |
| "Who calls this function?" | \`get_call_chain(symbol='<fqn>', direction='callers')\` |
| "Fix one function" | function-scope edit (see Operating Protocol §3) |
| "Refactor this file" | file-scope edit (see Operating Protocol §4) |
| "Change this subsystem" | \`get_module_info\` → \`get_file\` → \`get_dependencies\` → \`get_pr_insights\` |
| "Cross-cutting how does X work" | \`ask_codebase\` for synthesis + citations → drill into cited files with \`get_file\` |
...
/**
 * Create or update AGENTS.md in the project root with lgraph instructions.
