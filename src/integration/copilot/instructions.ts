/**
 * Instructions generation for Latentgraph GitHub Copilot integration.
 *
 * GitHub Copilot loads .github/copilot-instructions.md automatically as
 * project-level instructions. This file injects the mandatory lgraph MCP
 * usage rules into every Copilot session for the project, using section
 * markers to safely preserve user content outside the lgraph block.
 */
...
const COPILOT_INSTRUCTIONS_CONTENT = `${SECTION_MARKER}
...
- Don't search as the first step for indexed source-file understanding — \`get_file\` is faster and denser.
...
export function createCopilotInstructions(projectRoot: string): void {
    ...
    if (fs.existsSync(instructionsPath)) {
        ...
        if (existing.includes(SECTION_MARKER)) {
            ...
            console.log('  [copilot-instructions.md] Updated Latentgraph MCP instructions.');
            return;
        }
        ...
        console.log('  [copilot-instructions.md] Appended Latentgraph MCP instructions.');
    } else {
        const githubDir = path.join(projectRoot, '.github');
        if (!fs.existsSync(githubDir)) {
            fs.mkdirSync(githubDir, { recursive: true });
        }
        fs.writeFileSync(instructionsPath, COPILOT_INSTRUCTIONS_CONTENT);
        console.log('  [copilot-instructions.md] Created with Latentgraph MCP instructions.');
    }
}
