/**
 * Skill file generation for Latentgraph Kiro CLI integration.
 *
 * Generates .kiro/skills/lgraph-<name>/SKILL.md following the open agent
 * skills standard that Kiro CLI uses for automatic activation when a user
...
name: lgraph-cli
description: "Runs Latentgraph CLI commands for repo initialization, status checks, re-indexing, daemon control, and Kiro integration setup. Use when the user wants to index a repo, refresh the DRG, check lgraph status, start/stop the daemon, or configure Kiro. Triggers: \\"lgraph init\\", \\"index this repo\\", \\"refresh the DRG\\", \\"lgraph status\\", \\"re-scan\\", \\"configure Kiro\\", stale MCP results."
---
...
| \`lgraph add kiro\` | Configure Kiro MCP plus project guidance files |
| \`lgraph add kiro --yes\` | Same as above, but skip the consent prompt |
...
| Configure Kiro in the current repo | \`lgraph add kiro\` |
...
export function generateSkillFiles(projectRoot: string): GenerateSkillsResult {
    ...
    const skillsDir = path.join(projectRoot, '.kiro', 'skills');
